            ...
            if os.path.isfile(filename):
                ...
                if self.workstation_type == 'ch':
                    ...
                    for line in line_data:
                        ...
                        data_list.append(data)
                    data_list_list.append(data_list)
                    self.write_data(data_list_list, f"{dir_name}/raw_data")
        ...
        for folder in self.storage_data_dict.keys():
            if os.path.isdir(folder):
                ...
                for filename in os.listdir(raw_data_folder):
                    ...
                    print(f"\033[91mnormalize\033[0m of \033[34m{folder}\033[0m is done")


class Plot(Data):

    def __init__(self, workstation_type, plot_type, plot_style, dpi=300, color=False, latex=False, half=True):
        ...
        if plot_style in self.science_style_list:
            ...
            self.plot_style = getattr(matplotx.styles, style)
    ...
    def plt_plot(self, ax, data, half_index=None):
        ...
        for folder in self.storage_data_dict.keys():
            if os.path.isdir(folder):
                    ...
                    if self.workstation_type == 'kst':
                        if self.half:
                            for data, filename in zip(data_list, filename_list):
                                ...
                                self.plt_plot(all_ax, data, half_index=half_index)
                            ...
                            for data, filename in zip(data_list, filename_list):
                                ...
                                self.plt_plot(all_ax, data)
                                separated_ax.savefig(f"{pic_folder}/half_{filename[:-4]}.png", bbox_inches="tight")
                                separated_ax.cla()
                                separated_ax.close()
                            all_fig.savefig(f"{pic_folder}/half_all.png", bbox_inches="tight")
                            all_ax.cla()
                            all_ax.close(all_fig)

                    if self.workstation_type == 'ch':
                        for data, filename in zip(data_list, filename_list):
                            self.plt_plot(separated_ax, data)
                            self.plt_plot(all_ax, data)
                            separated_ax.savefig(f"{pic_folder}/{filename[:-4]}.png", bbox_inches="tight")
                            separated_ax.cla()
                            separated_ax.close()
                        all_ax.savefig(f"{pic_folder}/all.png", bbox_inches="tight")
                        all_ax.cla()
                        all_ax.close()
                ...
                if self.plot_type == 'ca':
                    for data, filename in zip(data_list, filename_list):
                        self.plt_plot(data)
                        plt.savefig(f"{pic_folder}/{filename[:-4]}.png", bbox_inches="tight")
                        plt.close()
                    total_data = np.vstack(data_list)
                    self.plt_plot(data)
                    plt.savefig(f"{pic_folder}/all.png", bbox_inches="tight")
                    plt.close()
            print(f"\033[91mplot\033[0m of \033[34m{folder}\033[0m is done")
